...
2. **Алерты «цена упала».** История цен уже копится в PostgreSQL (`price_history`), дашборд читает её из `/api/history`. Осталось построить график и сигналить о снижениях.
...
- **Хотите сами в код:** сделайте копию репозитория кнопкой Fork (это технический шаг для отправки PR, а не отдельная ветка развития: проект живёт здесь), склонируйте её к себе, создайте ветку (`git checkout -b my-feature`), внесите изменения и откройте Pull Request сюда.
